...
void nexterror(void) { longjmp(err_lab[--nerr_lab], 1); }
...
void rauth(void) { error("9660srv: authentication not required"); }
...
void rattach(void) {
  ...
  chat("attach(fid=%d,uname=\"%s\",aname=\"%s\")...", req->fid, req->uname,
       req->aname);
...
Xfile *doclone(Xfile *of, int newfid) {
  Xfile *nf, *next;
...
void rread(void) {
  ...
  if (f->qid.type & QTDIR)
    rep->count =
        (*f->xf->s->readdir)(f, (uint8_t *)fdata, req->offset, req->count);
  else
...
void rremove(void) { error("no removes"); }
...
void rwstat(void) { error("no wstat"); }

static int openflags(int mode) {
  int flags = 0;
...
mode=0x%8.8lux=0%luo atime=%s mtime=%s length=%lld uid=\"%s\" gid=\"%s\"...",
         s->name, s->qid.path, s->qid.vers, s->type, s->dev, s->mode, s->mode,
