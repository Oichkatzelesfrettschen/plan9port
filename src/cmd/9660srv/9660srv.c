...
static int iattach(Xfile *root);
static void iclone(Xfile *of, Xfile *nf);
static void iwalkup(Xfile *f);
static void iwalk(Xfile *f, char *name);
static void iopen(Xfile *f, int mode);
static void icreate(Xfile *f, char *name, long perm, int mode);
static long ireaddir(Xfile *f, uint8_t *, long, long);
static long iread(Xfile *f, char *, vlong, long);
static long iwrite(Xfile *f, char *, vlong, long);
static void iclunk(Xfile *f);
static void iremove(Xfile *f);
static void istat(Xfile *f, Dir *d);
static void iwstat(Xfile *f, Dir *d);

static char *nstr(uint8_t *, int);
static char *rdate(uint8_t *, int);
static int getcontin(Xdata *d, uint8_t *, uint8_t **);
...
static int iattach(Xfile *root) {
  ...
  if (getdrec(root, rd) >= 0) {
    ...
    s = (uint8_t *)rd->z.name + rd->z.namelen;
    if ((uintptr)s & 1) {
      s++;
      n--;
    }
    if (n >= 7 && s[0] == 'S' && s[1] == 'P' && s[2] == 7 && s[3] == 1 &&
        s[4] == 0xBE && s[5] == 0xEF) {
      root->xf->issusp = 1;
      root->xf->suspoff = s[6];
      n -= root->xf->suspoff;
...
static int casestrcmp(int isplan9, char *a, char *b) {
  int ca, cb;
 ...
 */
static long ireaddir(Xfile *f, uint8_t *buf, long offset, long count) {
  Isofile *ip = f->ptr;
  Dir d;
...
static long iread(Xfile *f, char *buf, vlong offset, long count) {
  int n, o, rcnt = 0;
  long size;
  vlong addr;
  Isofile *ip = f->ptr;
...
static long iwrite(Xfile *f, char *buf, vlong offset, long count) {
  USED(f);
  USED(buf);
  USED(offset);
  USED(count);
...
static void iclunk(Xfile *f) { USED(f); }
...
static int showdrec(int fd, int fmt, void *x) {
  Drec *d = (Drec *)x;
  int namelen;
  int syslen;
  ...
  if (d->z.reclen == 0)
    return 0;
  fprint(fd, "%d %d %ld %ld ", d->z.reclen, d->z.attrlen, l32(d->z.addr),
         l32(d->z.size));
  fprint(fd, "%s 0x%2.2x %d %d %ld ", rdate(d->z.date, fmt),
         (fmt == 'z' ? d->z.flags : d->r.flags), d->z.unitsize, d->z.gapsize,
         l16(d->z.vseqno));
...
static void newdrec(Xfile *f, Drec *dp) {
  Isofile *x = f->ptr;
  Isofile *n;
  int len;
...
static int getdrec(Xfile *f, void *buf) {
  Isofile *ip = f->ptr;
  int len = 0, boff = 0;
  ulong size;
  vlong addr;
...
static int opendotdot(Xfile *f, Xfile *pf) {
  uint8_t dbuf[256];
  Drec *d = (Drec *)dbuf;
...
static int rzdir(Xfs *fs, Dir *d, int fmt, Drec *dp) {
  ...
  s = (uint8_t *)dp->z.name + dp->z.namelen;
  if (((uintptr)s) & 1) {
    s++;
    sysl--;
  }
...
/*
 * Follow a continuation entry in the directory record.
 */
static int getcontin(Xdata *dev, uint8_t *p, uint8_t **s) {
  long bn, off, len;
  Iobuf *b;
...
/*
 * Format a byte string for debugging.
 */
static char *nstr(uint8_t *p, int n) {
  static char buf[132];
  char *q = buf;
...
/*
 * Convert an on-disk date to a printable string.
 */
static char *rdate(uint8_t *p, int fmt) {
  static char buf[64];
  int htz, s, n;

  n = sprint(buf, "%2.2d.%2.2d.%2.2d %2.2d:%2.2d:%2.2d", p[0], p[1], p[2], p[3],
             p[4], p[5]);
  if (fmt == 'z') {
    htz = p[6];
    if (htz >= 128) {
...
/*
 * Parse an ISO9660 7-byte timestamp into seconds.
 */
static long gtime(uint8_t *p) /* yMdhmsz */
{
  long t;
